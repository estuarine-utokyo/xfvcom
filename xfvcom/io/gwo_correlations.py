...
def get_cached_correlations(
    ...
    *,
    ...
    """
    Get correlations from cache or compute and cache them.

    This function automatically manages a correlation cache file. On first run,
    it computes correlations for all station pairs derived from fallback_stations
    and saves them to the cache. On subsequent runs, it loads from cache unless
    recompute=True.
    ...
    gwo_reader : GWOReader
        GWO data reader instance
    fallback_stations : dict[str, list[str]]
        Mapping of primary station to fallback stations
        e.g., {"Chiba": ["Tokyo", "Yokohama", "Tateyama"]}
    years : list[int] | None
        Years to use for correlation computation (default: 2015-2020)
    recompute : bool
        If True, force recomputation even if cache exists
    use_cache : bool
        If False, skip caching entirely (compute every time)

    Returns
    -------
    StationCorrelations
        Instance with correlations (from cache or freshly computed)

    Examples
    --------
    >>> from xfvcom.io.gwo_reader import GWOReader
    >>> reader = GWOReader("/path/to/GWO/Hourly")
    >>> fallback = {"Chiba": ["Tokyo", "Yokohama", "Tateyama"]}
    >>> corr = get_cached_correlations(reader, fallback)
    """
    if years is None:
    ...
    if use_cache and not recompute and cache_path.exists():
        try:
            ...
            # Verify cache has needed pairs
            needed_pairs = _get_station_pairs_from_fallback(fallback_stations)
            if _cache_has_all_pairs(cached, needed_pairs):
                print(f"[INFO] Loaded correlations from cache: {cache_path}")
                return cached
            print("[INFO] Cache exists but missing some station pairs, recomputing...")
        except Exception as e:
            print(f"[WARNING] Failed to load cache: {e}, recomputing...")

    # Compute correlations
    station_pairs = _get_station_pairs_from_fallback(fallback_stations)
    if not station_pairs:
        print("[WARNING] No station pairs to compute correlations for.")
        return StationCorrelations()

    print(f"[INFO] Computing correlations for {len(station_pairs)} station pairs...")
    print(f"[INFO] Using years: {years[0]}-{years[-1]}")

    correlations = StationCorrelations.compute(
        gwo_reader=gwo_reader,
    ...
    if use_cache:
        try:
            ...
            print(f"[INFO] Saved correlations to cache: {cache_path}")
        except Exception as e:
...
def _get_station_pairs_from_fallback(
    fallback_stations: dict[str, list[str]],
) -> list[tuple[str, str]]:
    """
    Generate station pairs from fallback station mapping.
    ...
    -------
    list[tuple[str, str]]
        List of (primary, secondary) pairs for correlation computation
    """
    pairs: list[tuple[str, str]] = []
    for primary, fallbacks in fallback_stations.items():
        for fallback in fallbacks:
            pairs.append((primary, fallback))
    return pairs


def _cache_has_all_pairs(
    cached: StationCorrelations, needed_pairs: list[tuple[str, str]]
) -> bool:
    """
    Check if cached correlations have all needed station pairs.

    Parameters
    ----------
    cached : StationCorrelations
        Cached correlations
    needed_pairs : list[tuple[str, str]]
        List of (primary, secondary) pairs needed

    Returns
    -------
    bool
        True if all pairs are present in cache
    """
    for primary, secondary in needed_pairs:
        pair_key = f"{primary}_{secondary}"
        if pair_key not in cached.correlations:
            return False
    return True
